...
export function GlobalHotkeyBinder({
  onOpenCommandPalette,
}: GlobalHotkeyBinderProps) {
  ...
  return (
    <>
      {Object.entries(configs).map(([actionId, config]) => {
        ...
        return <Hotkey key={actionId} keys={config.keys} handler={handler} />;
      })}
    </>
  );
}
...
function Hotkey({
  ...
  useHotkeys(normalized, handler, { enableOnFormTags: true }, [
    handler,
    normalized,
  ]);
  return null;
