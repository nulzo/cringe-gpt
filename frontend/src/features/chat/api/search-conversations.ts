...
const searchConversations = async (query: string, cursor?: string | null): Promise<ConversationSearchResponse> => {
  ...
  const items = (res.items || res.Items || []).map((it: any) => ({
    ...
    current_node_id: (it.currentNodeId ?? it.CurrentNodeId)?.toString?.() ?? null,
    ...
    payload: {
      ...
      message_id: (it.payload?.messageId ?? it.Payload?.MessageId)?.toString?.(),
...
export const useConversationSearch = (query: string) => {
  return useInfiniteQuery({
    ...
    queryFn: ({ pageParam }) => searchConversations(query, pageParam as string | undefined),
