...
export function SettingsModal({ open, onOpenChange }: SettingsModalProps) {
  ...
  useEffect(() => {
    if (!selectedProvider && initialProvider) setSelectedProvider(initialProvider);
  }, [selectedProvider, initialProvider]);

  const { data: providerSettings, isLoading: isLoadingProviderSettings } = useProviderSettings(selectedProvider as ProviderType);
  const { mutate: saveProviderSettings, isPending: isSavingProvider } = useUpdateProviderSettings(selectedProvider as ProviderType);
  const [apiUrl, setApiUrl] = useState<string>('');
  const [apiKey, setApiKey] = useState<string>('');
  const [defaultModel, setDefaultModel] = useState<string>('');
  const [showApiKey, setShowApiKey] = useState<boolean>(false);
  const providerMeta: Partial<Record<ProviderType, { label: string; description: string; urlPlaceholder: string; modelPlaceholder: string; docUrl: string }>> = {
    OpenAi: {
      label: 'OpenAI',
    ...
    Ollama: {
      label: 'Ollama',
      description: 'Run local models with Ollama. Ensure your Ollama daemon is running.',
    ...
    Anthropic: {
      label: 'Anthropic',
      description: 'Use Claude models for high-quality reasoning and assistance.',
  ...
  const { data: modelsForProvider = [], } = useModelsByProvider(selectedProvider);
  ...
  const handleSaveProvider = () => {
    ...
    saveProviderSettings(
      { apiKey: apiKey || undefined, apiUrl: apiUrl || undefined, defaultModel: defaultModel || undefined },
      {
        onSuccess: () => toast.success(`${selectedProvider} settings saved`),
        onError: () => toast.error(`Failed to save ${selectedProvider} settings`),
      }
    );
  };
  ...
  const handleClearProvider = () => {
    ...
    saveProviderSettings(
      { apiKey: '', apiUrl: '', defaultModel: '' },
      {
        ...
        onError: () => toast.error(`Failed to clear ${selectedProvider} settings`),
      }
    );
  };
  const isConfigured = (s?: { apiKey?: string; apiUrl?: string; defaultModel?: string }): boolean => {
    if (!s) return false;
    return Boolean((s.apiKey && s.apiKey.trim()) || (s.apiUrl && s.apiUrl.trim()) || (s.defaultModel && s.defaultModel.trim()));
  };
  const configuredByProvider = useMemo<Record<string, boolean>>(() => {
    const map: Record<string, boolean> = {};
    (providers || []).forEach((p) => {
      const cached = queryClient.getQueryData<ProviderSettingsType>(['providerSettings', p]);
      map[p] = isConfigured(cached);
    });
    return map;
  }, [providers, queryClient, providerSettings, apiUrl, apiKey, defaultModel]);
  const isDirty = useMemo(() => {
    return (apiUrl || '') !== (providerSettings?.apiUrl || '')
  ...
  const providerSections = useMemo(() => {
    return (providers || []).map((p) => ({ id: `provider:${p}`, label: providerMeta[p]?.label ?? p }));
  }, [providers]);
  ...
  const renderSectionContent = () => {
    switch (activeSection) {
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-6">
              ...
              {isLoadingProviders ? (
                ...
                <div className="text-muted-foreground text-sm">No providers available.</div>
              )}
                  ...
                  <div className="flex justify-between items-start">
                    <div>
                      <h4 className="font-semibold text-lg">
                        {providerMeta[selectedProvider]?.label ?? selectedProvider}
                      </h4>
                      <p className="mt-0.5 text-muted-foreground text-sm">
                        {providerMeta[selectedProvider]?.description ?? 'Configure credentials and defaults.'}
                      </p>
                    </div>
                    ...
                    <div className="gap-4 grid grid-cols-2">
                      <div className="space-y-2">
                        ...
                        <Input
                          ...
                          placeholder={providerMeta[selectedProvider]?.urlPlaceholder ?? 'https://...'}
                      ...
                      <div className="space-y-2">
                        ...
                        <Select value={defaultModel} onValueChange={setDefaultModel}>
                          <SelectTrigger id="default-model" className="h-10">
                            <SelectValue placeholder={providerMeta[selectedProvider]?.modelPlaceholder ?? 'model-id'} />
                          </SelectTrigger>
                          <SelectContent>
                            <SelectGroup>
      ...
      case "account":
        return (
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              <h5 className="text-xs font-semibold tracking-wide text-muted-foreground uppercase">Profile</h5>
              <div className="gap-4 grid grid-cols-2">
                <div className="space-y-2">
                  <Label>Email</Label>
            ...
            <div className="space-y-4">
              <h5 className="text-xs font-semibold tracking-wide text-muted-foreground uppercase">Preferences</h5>
              <div className="gap-4 grid grid-cols-2">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => updateSettings({ settings: { preferredModel: e.target.value || undefined } })}
                  />
                </div>
                <div className="space-y-2">
                  <Label>Theme</Label>
            ...
            <div className="space-y-2">
              <h5 className="text-xs font-semibold tracking-wide text-destructive uppercase">Danger Zone</h5>
              <div className="flex justify-between items-center border p-3 rounded-md">
                <div>
                  <div className="font-medium">Delete Account</div>
                  <div className="text-muted-foreground text-sm">Permanently remove your account and all associated data.</div>
                </div>
                <Button
                  variant="destructive"
                  onClick={async () => {
      ...
      default:
        if (activeSection.startsWith('provider:')) {
          const provider = activeSection.split(':')[1] as ProviderType | undefined;
          return (
            <div className="space-y-6">
              ...
              {isLoadingProviderSettings ? (
                ...
                <div className="space-y-6">
                  <div className="flex justify-between items-start">
                    <div>
                      <h4 className="font-semibold text-lg">{provider ? (providerMeta[provider]?.label ?? provider) : 'Provider'}</h4>
                      <p className="mt-0.5 text-muted-foreground text-sm">{provider ? (providerMeta[provider]?.description ?? 'Configure credentials and defaults.') : ''}</p>
                    </div>
                    {provider && providerMeta[provider]?.docUrl && (
                      <a className="text-primary hover:underline text-sm font-medium" href={providerMeta[provider]!.docUrl} target="_blank" rel="noreferrer">Docs</a>
                    )}
                  </div>
                  <Separator />
                  <div className="space-y-4">
                    <h5 className="text-xs font-semibold tracking-wide text-muted-foreground uppercase">Connection</h5>
                    <div className="gap-4 grid grid-cols-2">
                      <div className="space-y-2">
                        <Label htmlFor="api-url" className="text-sm font-medium">API URL</Label>
                        <Input id="api-url" value={apiUrl} onChange={(e) => setApiUrl(e.target.value)} placeholder={provider ? (providerMeta[provider]?.urlPlaceholder ?? 'https://...') : 'https://...'} disabled={!provider || isSavingProvider} className="h-10" />
                      </div>
                      <div className="space-y-2">
                        <Label htmlFor="default-model" className="text-sm font-medium">Default Model</Label>
                        <Select value={defaultModel} onValueChange={setDefaultModel}>
                          <SelectTrigger id="default-model" className="h-10">
                            <SelectValue placeholder={provider ? (providerMeta[provider]?.modelPlaceholder ?? 'model-id') : 'model-id'} />
                          </SelectTrigger>
                          <SelectContent>
                            <SelectGroup>
                              <SelectLabel>Models</SelectLabel>
                  ...
                  <div className="space-y-4">
                    <h5 className="text-xs font-semibold tracking-wide text-muted-foreground uppercase">Credentials</h5>
                    <div className="space-y-2">
                      <Label htmlFor="api-key" className="text-sm font-medium">API Key</Label>
                      <div className="relative">
                        <Input id="api-key" type={showApiKey ? 'text' : 'password'} value={apiKey} onChange={(e) => setApiKey(e.target.value)} placeholder="sk-..." disabled={!provider || isSavingProvider} className="pr-16 h-10" />
                        <button type="button" className="top-1/2 right-3 absolute text-xs text-muted-foreground hover:text-foreground font-medium -translate-y-1/2 transition-colors" onClick={() => setShowApiKey((v) => !v)}>
                          {showApiKey ? 'Hide' : 'Show'}
                        </button>
                      </div>
                    </div>
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="flex gap-0 p-0 max-w-3xl h-[70vh] overflow-hidden">
          ...
          <nav className="flex flex-col space-y-1">
            {primarySections.map((section) => {
              ...
              return (
                ...
                >
                  <span className="inline-flex items-center gap-2"><Icon className="size-5 text-foreground/90" />{section.label}</span>
                </Button>
              );
            })}
            ...
            {!!providerSections.length && (
              <>
                <div className="px-2 mb-1 text-xs font-semibold tracking-wide text-muted-foreground uppercase">Providers</div>
                {providerSections.map((section) => {
                  ...
                  return (
                    <Button
                      ...
                      onClick={() => setActiveSection(section.id)}
                    >
                      <span className="inline-flex items-center gap-2"><ProviderIconFromKey providerKey={key} size={18} className="rounded" />{section.label}</span>
                    </Button>
                  );
                })}
                <Separator className="my-3" />
            ...
            <div className="px-2 mb-1 text-xs font-semibold tracking-wide text-muted-foreground uppercase">More</div>
        ...
        <div className="flex-1 p-6 overflow-y-auto">
          ...
          {(activeSection === 'ai' || activeSection.startsWith('provider:')) && !!selectedProvider && isDirty && (
